...
to setup
  ...
  [
    set pcolor blue
    ask patches in-radius random 4 [set pcolor blue]
  ]
  ask n-of 10 patches
  [
    set pcolor yellow
    ask patches in-radius random 4 [set pcolor yellow]
  ]
  ; Se the valid patches (not wall)
  set p-valids patches with [pcolor != blue]
  ; Create a random start
  set Start one-of p-valids
  ask Start [set pcolor red]
  set Start1 one-of p-valids
  ask Start1 [set pcolor red]
  set Goal one-of p-valids
  ask Goal [set pcolor green]
  set Goal1 one-of p-valids
  ask Goal1 [set pcolor green]
...
;****************************************************************************************************************************************
to-report A_etoile [#Start  #Goal #Goal1  #valid-map]
  ; clear all the information in the agents
  ask #valid-map with [visited?]
  [
    set father nobody
  ...
  ; a path exists
  ;***************************************************
  if Total-expected-cost Goal < Total-expected-cost Goal1
  [
  while [not [visited?] of #Goal and exists?]
  [
    ; We only work on the valid pacthes that are active
  ...
  [
    ...
    ; by jumping back from #Goal to #Start by using the fathers of every patch
    let current #Goal1
    set Final-Cost (precision [Cost-path] of #Goal1 3)
    let rep (list current)
    While [current != #Start]
    [
      set current [father] of current
  ...
  ;*******************************************************
  if Total-expected-cost Goal1 < Total-expected-cost Goal
  [
  while [not [visited?] of #Goal1 and exists?]
  [
    ; We only work on the valid pacthes that are active
    let options #valid-map with [active?]
    ; If any
    ...
    [
      ; Take one of the active patches with minimal expected cost
      ask min-one-of options [Total-expected-cost #Goal1]
      [
        ; Store its real cost (to reach it) to compute the real cost
        ; of its children
        let Cost-path-father Cost-path
        ...
        [
          ...
          ; first case an upper bound big enough to be sure that the new path
          ; will always be smaller.
          let t ifelse-value visited? [ Total-expected-cost #Goal1] [2 ^ 20]
          ; If this temporal cost is worse than the new one, we substitute the
          ; information in the patch to store the new one (with the neighbors
          ; of the first case, it will be always the case)
          if t > (Cost-path-father + distance myself + Heuristic #Goal1)
          [
            ; The current patch becomes the father of its neighbor in the new path
            set father myself
            set visited? true
...
to solve
  ...
  let path1  A* Start1  Goal1  p-valids
...
to clean
  cd
  ask patches with [pcolor != black and pcolor != blue] [set pcolor black]
  ask Start [set pcolor red]
end
@#$#@#$#@
GRAPHICS-WINDOW
