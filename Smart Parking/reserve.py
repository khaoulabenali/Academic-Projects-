...
class CustomDialog3(QDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.btn_Add = QPushButton('RESERVE A PLACE')
        ...
        if (little) and ( len(txt_mark) >= 1 ) and ( 1<= L < 5) and (res):
            ...
            if len(cursor.fetchall()) <= 5:
                ...
                cursor.execute(""" INSERT INTO Parking(fmark,fnumber,flittle,fmdp,fexists,ftime,fplace)
                            VALUES(?,?,?,?,?,?,?) """,
                               (txt_mark, txt_number, self.__littlecar, passwd, 0, time_Add, place))
                text = "WELCOME , YOU HAVE RESERVED PLACE N°" + str(place) + "\n WOULD YOU PLEASE TAKE YOUR PASSWORD "
            ...
            if len(cursor.fetchall()) <= 5:
                ...
                cursor.execute(""" INSERT INTO Parking(fmark,fnumber,flittle,fmdp,fexists,ftime,fplace) 
                            VALUES (?,?,?,?,?,?,?)""",
                               (txt_mark, txt_number, self.__bigcar, passwd, 0, time_Add, place))
                text = "WELCOME , YOU HAVE RESERVED PLACE N°" + str(place) + "\n WOULD YOU PLEASE TAKE YOUR PASSWORD "
                self.pop_message(text)
                self.pop_message(passwd)
                conn.commit()
